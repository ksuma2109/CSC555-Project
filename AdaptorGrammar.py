...
class AdaptorGrammar(nn.Module):
  def __init__(self, vocab = 100,
    ...
    if z_dim > 0:
      ...
      self.ag_emb = nn.Embedding(ant_states, w_dim)
  ...
  def forward(self, x, argmax=False, use_mean=False):
    ...
    ant_emb = ant_emb.unsqueeze(0).expand(batch_size, self.ant_states, self.state_dim)
    ...
    adapted_rule_score = F.log_softmax(self.adpt_mlp(ant_emb), 2)
    rule_scores.scatter_add_(1, adapted_rule_score, ant_indices.unsqueeze(1).expand(batch_size, :))
